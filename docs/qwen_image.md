...
- For Qwen-Image-Edit control-image captions, set `control_caption_extension` in the image dataset config. For example, `control_caption_extension = ".caption.txt"` reads `control1.caption.txt` for `control1.png`.
...
- Qwen-Image-Editで制御画像キャプションを使う場合は、画像データセット設定に`control_caption_extension = ".caption.txt"`のように指定してください。`control1.png`に対して`control1.caption.txt`を読み込みます。
...
python src/musubi_tuner/qwen_image_generate_image.py \
    ...
    --control_image_caption "animation screenshot image" \
...
- For Qwen-Image-Edit:
    ...
    - `--control_image_caption`: Optional caption(s) for the control image(s), matched by order. For numbered-image Qwen prompts, these are injected as `Picture N: caption <image token>`.
    - `--control_image_caption_mode`: `append` adds control image captions inside each `Picture N: caption <image token>` line; `replace` uses only the control image captions. The default is `append`.
...
- Qwen-Image-Editの場合：
    ...
    - `--control_image_caption`: 制御画像ごとの任意のキャプションです。`--control_image_path`と同じ順序で指定し、番号付き画像のQwenプロンプトでは`Picture N: caption <image token>`として追加されます。
    - `--control_image_caption_mode`: `append`は各制御画像キャプションを対応する`Picture N: caption <image token>`行の中に追加し、`replace`は制御画像キャプションのみを使用します。デフォルトは`append`です。
